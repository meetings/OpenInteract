...
# $Id: SiteTemplate.pm,v 1.14 2004/12/05 08:51:20 lachoy Exp $
...
$OpenInteract2::Action::SiteTemplate::VERSION = sprintf("%d.%02d", q$Revision: 1.14 $ =~ /(\d+)\.(\d+)/);
...
sub list {
    ...
    my $tmpl_class = CTX->lookup_class( 'template' );
    my %templates = ();
...
sub update {
    ...
    eval { $template->save };
    if ( $@ ) {
        $self->add_error_key( 'base_template.error.save', $@ );
