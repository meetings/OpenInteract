...
sub new {
    ...
    my %data = map { $_ => $p->{ $_ } } @VALID_INIT_KEYS;
...
sub save {
    ...
    eval {
        ...
        $out->open( "> $results_file" )
                    || die "Cannot open for writing ($results_file): $!\n";
        ...
        $meta_out->open( "> $meta_file" )
                         || die "Cannot open for writing ($meta_file): $!\n";
...
sub retrieve_raw_results {
    ...
    while ( <RESULTS> ) {
        ...
        chomp;
        my ( $item_class, $item_value, @extra ) = split /$RECORD_SEP/, $_, 2 + $meta_info->{num_extra};
...
sub assign_results_to_object {
    ...
    for ( qw( num_records time date extra_name num_extra record_class result_list ) ) {
...
sub get_results_dir {
    my ( $class ) = @_;
    return CTX->server_config->{dir}{overflow};
}
...
sub build_results_filename {
    ...
    $self->{results_dir} ||= $self->get_results_dir;
    return join( '/', $self->{results_dir}, $search_id );
}
...
sub build_meta_filename {
    ...
    $self->{results_dir} ||= $self->get_results_dir;
    return join( '/', $self->{results_dir}, "$search_id.meta" );
}
...
sub build_lock_filename {
    ...
    $self->{results_dir} ||= $self->get_results_dir;
    return join( '/', $self->{results_dir}, "$search_id.$LOCK_EXT" );
}
...
sub results_lock {
    ...
    open( LOCK, "> $lock_file" )
        || die "Cannot open lockfile ($lock_file): $!";
