...
process; in addition to installing the various modules it will copy
the script C<oi2_manage> to somewhere in your path.
...
And modify the server configuration to use them:
...
Also in the daemon startup directory is a file C<oi2.pid> holding the
daemon's process ID. On unix-y systems you can use this to kill the
process:
