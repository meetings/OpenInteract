...
C<book/OpenInteract2/Action/Book.pm>:
  ...
  20: # deal with it :-)
  21: 
  22: $OpenInteract2::Action::Book::VERSION = sprintf("%d.%02d", q$Revision: 1.26 $ =~ /(\d+)\.(\d+)/);
  23: 
...
C<book/OpenInteract2/Action/Book.pm>: will end up looking like this:
  ...
  $OpenInteract2::Action::Book::VERSION = sprintf("%d.%02d", q$Revision: 1.26 $ =~ /(\d+)\.(\d+)/);
...
use to display how many entries we found. (Note that 'total_hits' is
only provided when you're using paged results, but since that's the
default and we didn't change the setting we can use the parameter. See
the docs for more.)
