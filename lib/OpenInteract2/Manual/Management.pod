...
Its size also meant that it was difficult to refactor. And it was
fairly difficult to add new tasks without doing lots of copy-and-paste
and knowing where certain tasks (parameter parsing, etc.) were
performed.
...
This management task provides C<_get_source_dir_param()> which can be
used as necessary:
