...
sub get_param_description {
    ...
    if ( $param_name eq 'website_dir' ) {
        return "Directory where website will be created. It must " .
...
sub _check_website_dir {
    ...
    if ( -d $website_dir ) {
        return "Website directory [$website_dir] already " .
               "exists; cannot create website over an " .
               "existing site";
    }
}
...
sub run_task {
    ...
    $self->setup_context({ skip => 'initialize repository' });
